...
#[error("value must be between 0 to 63")]
pub struct SquareFromU8Error;
...
impl TryFrom<u8> for Square {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        (0..64)
            .contains(&value)
            .then_some(Square(value))
            .ok_or(SquareFromU8Error)
    }
}

#[derive(Debug, thiserror::Error)]
#[error("bitboard must have a single on bit")]
pub struct SquareFromBitBoardError;

impl TryFrom<BitBoard> for Square {
    type Error = SquareFromBitBoardError;

    fn try_from(value: BitBoard) -> Result<Self, Self::Error> {
        value
            .is_a_single_one()
            .then(|| value.first_one_as_square().unwrap())
            .ok_or(SquareFromBitBoardError)
    }
}

impl Display for Square {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ((b'a' + self.file()) as char).fmt(f)?;
        (self.rank() + 1).fmt(f)?;
